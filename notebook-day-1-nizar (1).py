...
def _(M, g, l, np, plt, redstart_solve):
    def controlled_landing_example():
        T = 5.0
        # contraintes : y(0)=10, y'(0)=-2, y''(0)=-g  et  y(T)=l/2, y'(T)=0, y''(T)=0
        a0, a1, a2 = 10.0, -2.0, -g / 2.0  # car y(t) = sum a_k t^k => y''(0) = 2*a2
        rhs = np.array(
            [
                l / 2.0 - a0 - a1 * T - a2 * T**2,
                0.0 - a1 - 2 * a2 * T,
                0.0 - 2 * a2,
            ]
        )
        ...
        axes[0].axhline(l / 2.0, color="grey", ls="--", label=r"$y = \ell/2$")
...
def _(M, g, l, np, svg, transform):
    ...
    def booster(x, y, theta, f, phi):
        # Longueur de flamme : proportionnelle à f, vaut l/2 quand f = M*g.
